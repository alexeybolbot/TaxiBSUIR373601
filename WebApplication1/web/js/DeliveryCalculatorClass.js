    ...
    function (provide, defineClass) {
        ...
        defineClass(DeliveryCalculator, {
            ...
            geocode: function (pointType, point) {
                ymaps.geocode(point).then(function (result) {
                    ...
                    if (pointType == 'start') {
                        ...
                        var adress = result.geoObjects.get(0).properties.get('balloonContentBody'); 
                        var adress2 = ""; 
                        var i, i2 = 0; 
                        for(i=0; i<adress.length;i++){ 
                        if(adress[i] == '>') {i2 = 1; i++;} 
                        if(i2 == 1 && adress[i] == '<') {i2 = 3; i = adress.length} 
                        if(i2 == 1) adress2 += adress[i]; 
                        } 
                        document.getElementById('otkudaOrder').value = adress2;
                        ...
                        document.getElementById('kudaOrder').value = adress2;
            ...
            calculate: function (routeLength) {
                // Константы.
                var DELIVERY_TARIF = 5900, // Стоимость за километр.
                    MINIMUM_COST = 20000; // Минимальная стоимость.
                ...
                if (this._startPoint && this._finishPoint) {
                    ...
                    ymaps.route([start, finish])
                        .then(function (router) {
							...
							document.getElementById('SUMMA').value=distance*5900;
                                                        //document.getElementById('otkudaOrder').value=$("input[name=]").val();
                                                        //document.getElementById('kudaOrder').value=finish;
